...
function render(root, { L }) {
  ...
  root.append(card(T('title'), T('desc'), (body) => {
    ...
    function draw(st) {
      ...
      box.append(tbl, h('div', { class: 'note' },
        L({ en: 'Verified vs Kruger official catalogue: TDA-L 315-1400 mm, Class F motor, min. IP55. Full catalogue: https://www.krugerfan.com.cn/', zh: '已對照 Kruger 官方型錄核實：TDA-L 315–1400 mm、F 級電機 IP55。完整型錄：https://www.krugerfan.com.cn/' })));
    }
    function rebuild() {
      const sList = statics();
  ...
  root.append(card(L({ en: 'EAF Selection Sheets (Fantech · Anway HK)', zh: 'EAF 選型單（Fantech · Anway 香港代理）' }), '', (body) => {
    ...
    body.append(h('div', { class: 'note' },
      L({ en: '26 real selection sheets from the catalogue library (Fantech HXM propeller / PowerLine centrifugal / HCE, 240 V 50 Hz, 4-pole). Source: G:\\我的雲端硬碟\\catalogue\\Ventilation Fan (EAF-*.pdf).', zh: '26 份真實選型單（Fantech HXM 軸流／PowerLine 離心／HCE，240 V 50 Hz、4 極）。來源：G:\\我的雲端硬碟\\catalogue\\Ventilation Fan（EAF-*.pdf）。' })));
  }, { src: 'Ventilation Fan (EAF-*.pdf) — Fantech, HK rep. Anway Engineering', collapsed: true }));
}
