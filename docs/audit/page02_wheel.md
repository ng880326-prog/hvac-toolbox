...
`app/js/modules/wheel.js` 已按上表重建，六張卡依原檔次序：系統條件 → 設計條件（4 行 × 9 欄）
→ 風量（含 0.7–1.5 範圍警告）→ 回收效率 → 結果並排（夏季｜冬季）→ 轉輪示意圖（原檔 B21:F37 重繪）。

### 與原檔的差異（每項都因為原檔值不可能成立）

| 項目 | 原檔 | 本模組 | 理由 |
|---|---|---|---|
| ρ·cp | ρ × 1.02 = 1.224 | ρ × 1.006 | cp(乾空氣) 標準值；差異 <1.5% |
| ηS / ηT | 快取 2354 / 2356（格式 0.0%，即 235400%） | 0.75 / 0.70，可改 | 原檔殘值無效 |
| Vs / Ve | 1132，標籤 `m3/s`（＝4,075,200 m³/h） | 1132 L/s 為預設，可選 L/s｜m³/s｜m³/h | 只有 L/s 才合理 |
| 壓力 | 固定 101.325 kPa | 由高度以 ISA 公式推算，可手動覆寫 | 原檔有高度欄但不影響計算 |
| 三欄入風（DB/WB/RH） | 三者並存，可互相矛盾（原檔 Air2 40% RH 就是如此） | 以最後修改者為準，另一項重算並在 >1.5% 時發警示 | 消除自相矛盾輸入 |
| 回收量正負 | 夏欄用 (t_o,1 − t_r,1)、冬欄用 (t_r,1 − t_o,1) | 一律取正數（大小），方向由欄位決定 | 兩欄同為「回收量」，避免負號誤讀 |

### 驗證證據

- `node tests/run_tests.mjs` → **PASS 76 / 76**（含新增 `WfromEnthalpyT` 反算向量）。
- `node tests/smoke.mjs` → **SMOKE OK**（wheel 卡 6 張、30 個結果格）。
- `python tools/verify_wheel.py`（獨立用 Python 重寫 ASHRAE 濕空氣＋原檔轉輪代數，再於真實瀏覽器
  讀取每個結果格）→ **22 ok / 0 fail**：夏季 QT 37.42 kW、QS 10.63、QL 26.80、供風出口
  26.75 °C／20.85 °C WB／60.17 kJ/kg、排風出口 32.25 °C／25.20 °C WB／76.87 kJ/kg；
  冬季（顯熱輪）QS 17.28、QT 17.39、供風出口 17.75 °C、排風出口 9.25 °C；冬季排風出口
  9.25 °C < 露點 11.1 °C → 正確觸發原檔「**Condensation occurs at exhaust outlet」警告。
- `python tools/check_wheel_layout.py` → 桌面 1440 與平板 820 **兩欄並排**（x=317/862 同 y）、
  手機 390 **自動上下堆疊**、頁面無橫向溢出、0 console error；示意圖窄屏版本 320×264（字級可讀）。
- 截圖：`docs/verification/wheel_{desktop,mobile,tablet}{,_table,_schematic}.png`。
